...
import static ic.test.comparison.ComparisonUtilityTest.getFolder;
...
public class TestComparisonScenarioGeneratorWithMappings extends TestCase {
    ...
    public void testExecutionSmall() {
...
//        String[] datasets = {"conference"};
    ...
    public void testExecutionGreedy() {
...
//        String[] datasets = {"conference", "doctors-100", "doctors-1k"};
    ...
    public void testExport() {
        System.out.println("");
        // "conference", "doctors-100",
        String[] datasets = {"doctors-100k"};
//        String[] datasets = {"bikeshare"};
//        String[] datasets = {"conference"};
        String exportPath = "/Users/donatello/Desktop/instance-comparisons/";
        for (String dataset : datasets) {
            System.out.println(dataset);
...
//        String[] datasets = {"conference"};
        ...
        for (int cellsToChangeValue : cellsToChangeValues) {
            ...
            String exportPath = "~/Desktop/instance-comparisons/exp-diff-errors-new-strat/" + this.cellsToChange + "-" + this.newReduntandTuples + "-" + this.newRandomTuples + "-new/";
            for (String dataset : datasets) {
                System.out.println(dataset);
...
//            addRandomRowsInSourceAndTarget(dataset, new ComputeInstanceSimilarityHashing(useBruteForceInGreedy), true, exportPath);
    ...
    private void modifyCellsInSource(String scenarioName, IComputeInstanceSimilarity similarityChecker, boolean computeSimilarity, String expPath) {
        ...
        InstancePair instancePair = generator.generateWithMappings(getFolder(sourceFile, "/redundancy/", false), true, false);
    ...
    private void modifyCellsInTarget(String scenarioName, IComputeInstanceSimilarity similarityChecker, boolean computeSimilarity, String expPath) {
        if (similarityChecker == null) {
            similarityChecker = new ComputeInstanceSimilarityBruteForce();
        }
        String sourceFile = scenarioName + "/initial/";
        setInjectiveFunctionalMapping();
        ComparisonScenarioGeneratorWithMappings generator = new ComparisonScenarioGeneratorWithMappings(0, 0, cellsToChange, 1234);
        InstancePair instancePair = generator.generateWithMappings(getFolder(sourceFile, "/redundancy/", false), false, true);
        IDatabase sourceDB = instancePair.getLeftDB();
        IDatabase targetDB = instancePair.getRightDB();
        List<TupleWithTable> sourceTuples = SpeedyUtility.extractAllTuplesFromDatabaseForGeneration(sourceDB);
        List<TupleWithTable> targetTuples = SpeedyUtility.extractAllTuplesFromDatabaseForGeneration(targetDB);
    ...
    private void modifyCellsInSourceAndTarget(String scenarioName, IComputeInstanceSimilarity similarityChecker, boolean computeSimilarity, String expPath) {
//        String scenarioName = "conference";
        ...
        setInjectiveFunctionalMapping();
        ComparisonScenarioGeneratorWithMappings generator = new ComparisonScenarioGeneratorWithMappings(0, 0, cellsToChange, 1234);
        InstancePair instancePair = generator.generateWithMappings(getFolder(sourceFile, "/redundancy/", false), true, true);
    ...
    private void addRedundantRowsInSource(String scenarioName, IComputeInstanceSimilarity similarityChecker, boolean computeSimilarity, String expPath) {
        if (similarityChecker == null) {
            similarityChecker = new ComputeInstanceSimilarityBruteForce();
        }
        String sourceFile = scenarioName + "/initial/";
    ...
    private void addRedundantRowsInTarget(String scenarioName, IComputeInstanceSimilarity similarityChecker, boolean computeSimilarity, String expPath) {
        if (similarityChecker == null) {
            similarityChecker = new ComputeInstanceSimilarityBruteForce();
        }
        String sourceFile = scenarioName + "/initial/";
    ...
    private void addRedundantRowsInSourceAndTarget(String scenarioName, IComputeInstanceSimilarity similarityChecker, boolean computeSimilarity, String expPath) {
        if (similarityChecker == null) {
            similarityChecker = new ComputeInstanceSimilarityBruteForce();
        }
        String sourceFile = scenarioName + "/initial/";
    ...
    private void addRandomRowsInSource(String scenarioName, IComputeInstanceSimilarity similarityChecker, boolean computeSimilarity, String expPath) {
        if (similarityChecker == null) {
            similarityChecker = new ComputeInstanceSimilarityBruteForce();
        }
        String sourceFile = scenarioName + "/initial/";
    ...
    private void addRandomRowsInTarget(String scenarioName, IComputeInstanceSimilarity similarityChecker, boolean computeSimilarity, String expPath) {
        if (similarityChecker == null) {
            similarityChecker = new ComputeInstanceSimilarityBruteForce();
        }
        String sourceFile = scenarioName + "/initial/";
    ...
    private void addRandomRowsInSourceAndTarget(String scenarioName, IComputeInstanceSimilarity similarityChecker, boolean computeSimilarity, String expPath) {
        if (similarityChecker == null) {
            similarityChecker = new ComputeInstanceSimilarityBruteForce();
        }
        String sourceFile = scenarioName + "/initial/";
        setNonInjectiveNonFunctionalMapping();
//        IDatabase originalDB = loadDatabase(sourceFile, "/resources/redundancy/");
        ComparisonScenarioGeneratorWithMappings generator = new ComparisonScenarioGeneratorWithMappings(0, newRandomTuples, cellsToChange, 1234);
        InstancePair instancePair = generator.generateWithMappings(getFolder(sourceFile, "/redundancy/", false), true, true);
        IDatabase sourceDB = instancePair.getLeftDB();
        IDatabase targetDB = instancePair.getRightDB();
...
//            ComparisonConfiguration.setForceExaustiveSearch(false);
    ...
    private void addRandomAndRedundantRowsInSourceAndTarget(String scenarioName, IComputeInstanceSimilarity similarityChecker, boolean computeSimilarity, String expPath) {
        if (similarityChecker == null) {
            similarityChecker = new ComputeInstanceSimilarityBruteForce();
        }
        String sourceFile = scenarioName + "/initial/";
