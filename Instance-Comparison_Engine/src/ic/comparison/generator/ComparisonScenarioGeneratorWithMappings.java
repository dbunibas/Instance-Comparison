package ic.comparison.generator;
...
public class ComparisonScenarioGeneratorWithMappings {
    ...
    private long lastPlaceholderIdRight = 1;
    private long timeGeneration = 0;
    private Map<AttributeRef, Map<IValue, NullValue>> cachePlaceholderLeft = new HashMap<>();
    private Map<AttributeRef, Map<IValue, NullValue>> cachePlaceholderRight = new HashMap<>();
    ...
    public InstancePair generateWithMappings(String originalDBPath, boolean changeSource, boolean changeTarget) {
        IDatabase originalDB = ICUtility.loadMainMemoryDatabase(originalDBPath);
        long start = System.currentTimeMillis();
        initOperators(originalDB);
        oidGenerator.initializeOIDs(originalDB);
        IDatabase leftDB = dbManager.cloneTarget(ICUtility.loadMainMemoryDatabase(originalDBPath), "_left");
...
//        tupleMapping.updateValueMappings(); // clean and replace value mappings
    ...
    private void modifyCells(List<TupleWithTable> sourceTuples, List<TupleWithTable> targetTuples, TupleMapping tupleMapping, IDatabase db, boolean isLeft) {
        ...
        for (int pos = 0; pos < sourceTuples.size(); pos++) {
            ...
            TupleWithTable modifiedTuple = modifyTuple(db, originalTuple, isLeft);
...
//                List<TupleMatch> previousMatches = getPreviousMatches(originalTuple, tupleMapping);
    ...
    private TupleWithTable modifyTuple(IDatabase db, TupleWithTable originalTuple, boolean isLeft) {
        ...
        for (Cell cell : cellsToChangeInSource) {
            ...
            IValue newValue = generateNewValueForModify(attribute.getType(), isLeft, cell, attributeRef);
...
//                }
    ...
    private IValue generateNewValueForModify(String type, boolean isLeft, Cell cell, AttributeRef attribute) {
        if (random.nextBoolean()) { //New Constant
            ...
            return getNextPlaceholder(isLeft, cell.getValue(), attribute);
        }
    }
...
//            int randomLimitedInt = leftLimit + (int) (new Random().nextFloat() * (rightLimit - leftLimit));
    ...
    private NullValue getNextPlaceholder(boolean isLeft, IValue oldValue, AttributeRef attribute) {
        //lastPlaceholderId++;
        //return new NullValue(SpeedyConstants.getStringSkolemPrefixes()[0] + lastPlaceholderId);
        Map<IValue, NullValue> nullsForAttribute = cachePlaceholderLeft.get(attribute);
        if (nullsForAttribute == null) {
            nullsForAttribute = new HashMap<>();
            cachePlaceholderLeft.put(attribute, nullsForAttribute);
        }
        if (!isLeft) {
            nullsForAttribute = cachePlaceholderRight.get(attribute);
            if (nullsForAttribute == null) {
                nullsForAttribute = new HashMap<>();
                cachePlaceholderRight.put(attribute, nullsForAttribute);
            }
        }
        NullValue placeholder = nullsForAttribute.get(oldValue);
        if (placeholder != null) {
            return placeholder;
        }
        if (isLeft) {
            lastPlaceholderId++;
            placeholder = new NullValue(SpeedyConstants.getStringSkolemPrefixes()[0] + lastPlaceholderId);
        } else {
            lastPlaceholderIdRight++;
            placeholder = new NullValue(SpeedyConstants.getStringSkolemPrefixes()[0] + lastPlaceholderIdRight);
        }
        nullsForAttribute.put(oldValue, placeholder);
        return placeholder;
    }
