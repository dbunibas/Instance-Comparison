...
import ic.ICConstants;
import ic.comparison.ComparisonConfiguration;
import static ic.comparison.ComparisonConfiguration.getInstance;
...
public class ICUtility {
    ...
    private final static Logger logger = LoggerFactory.getLogger(ICUtility.class);
    ...
    public static String printIterator(ITupleIterator iterator) {
        StringBuilder result = new StringBuilder();
        while (iterator.hasNext()) {
            result.append(ICConstants.INDENT).append(iterator.next().toStringWithOID()).append("\n");
        }
        iterator.reset();
        return result.toString();
    }
    ...
    public static Tuple createTuple(INode tupleNode, String tableName) {
        ...
        Cell oidCell = new Cell(tupleOID, new AttributeRef(tableName, ICConstants.OID), new ConstantValue(tupleOID));
    ...
    public static IValue getOriginalOid(Tuple tuple, TableAlias tableAlias) {
        Cell oidCell = tuple.getCell(new AttributeRef(tableAlias, ICConstants.OID));
    ...
    public static String getDeltaRelationName(String tableName, String attributeName) {
        return tableName + ICConstants.DELTA_TABLE_SEPARATOR + attributeName;
    }
